...
- `batch_size`: batch size used for this run execution
...
Fields:
- `run_id`: parent run identifier
- `batch_size`: optional batch size context for run-level events; required for epoch events
...
Represents markdown reporting output generated by `analyze.py`.

Fields:
- `report_path`: markdown file path (`results.md`)
- `generated_at`: report generation timestamp
- `config_device`: compared device scope
- `config_epochs`: epoch count used for compared rows
- `config_learning_rate`: learning rate used for compared rows
- `config_batch_sizes`: list of compared batch sizes
- `table_final_metrics`: final train/validation/test loss+accuracy by batch size
- `table_epoch_sampled`: per-batch sampled metrics at epoch multiples of 10 plus final epoch
- `historical_scope`: indicates report includes matching historical rows from results directory
...
- One `ResultsReport` aggregates many `TrainingRun` rows filtered from historical CSV data in a results directory.
...
- `results.md` must contain both required comparison tables and configuration metadata.
- Epoch-sampled report rows include multiples of 10 and always include final epoch.
